function y = hazardfunc_single_malignant_cells_brc(t,x,p,N0,Menarche_Age)
    ...
    y(5) = Delay*(beta2 - (alpha2+beta2+mu2)*x(5)+mu1*x(5)*x(7)+alpha2*x(5)^2);
    y(6) = Delay*(-(alpha2+beta2+mu2)*x(6)+mu2*x(6)*x(7)+mu2*x(5)*x(8)+2*alpha2*x(5)*x(6));
    y(7) = Delay*(beta3-(alpha3+beta3+mu3)*x(7)+alpha3*x(7)^2);
    y(8) = Delay*(-(alpha3+beta3+mu3)*x(8)+2*alpha3*x(8)*x(7));
end
