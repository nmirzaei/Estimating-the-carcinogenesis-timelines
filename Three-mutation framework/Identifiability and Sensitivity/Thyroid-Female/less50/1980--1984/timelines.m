function y = timelines(PARAMS,N0,T,Age)
        ...
        f_FSMC = P0_prime_1 / trapz(t_span, P0_prime_1); % density for normal → FSMC
    
        f_SSMC_given_FSMC = P1_prime_2 / trapz(t_span, P1_prime_2);%P0_prime_2 / trapz(t_span, P0_prime_2); % FSMC → SSMC
        f_SSMC = conv(f_FSMC,f_SSMC_given_FSMC)*dt;
        f_SSMC = f_SSMC(1:length(t_span));
        f_SSMC = f_SSMC/trapz(t_span,f_SSMC);

        f_Malignant_given_SSMC = P2_prime_3 / trapz(t_span, P2_prime_3);%+P0_prime_3 / trapz(t_span, P0_prime_3) + P1_prime_3 / trapz(t_span, P1_prime_3) % SSMC → malignancy
