...
UB=[1E-5 1E-2 1 10 10 10 10];  %upper allowed parameter bounds
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Building paremter grids for the profile sweep
%Note: The scale values are picked heuristically to zoom the plots to
%desired range since the parameters have different magnitude scales
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
scale_1 = 0.35;
scale_2 = 0.35;
scale_3 = 1.8;
...
for i = 1:length(theta1)
    warning('off', 'all')
    p_ = theta1(i);
    profile_params = fmincon(@(p1) ODECalc_hazard_SimpleBirth([p1(1:i-1), p_, p1(i:end)],T,data1,Cell_num_array(:,3),Age,N0,bm,Menarche_Age), [p_guess(1:i-1),p_guess(i+1:end)],[],[],[],[],[LB(1:i-1),LB(i+1:end)],[UB(1:i-1),UB(i+1:end)],[],opts_);

    for j=1:K
        try
            profile_likelihoods(i,j) = ODECalc_hazard_SimpleBirth([profile_params(1:i-1), p_pert(i,j), profile_params(i:end)],T,data1,Cell_num_array(:,3),Age,N0,bm,Menarche_Age);
        catch
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
loss_values = ODECalc_hazard_SimpleBirth(theta1,T,data1,Cell_num_array(:,3),Age,N0,bm,Menarche_Age);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:length(theta1)
    ...
    box on
    hold on
    plot(p_pert(i,:),numel(data1)*log(smoothdata(profile_likelihoods(i,:)/loss_values,'movmean',3)),'LineWidth',2.5)
...
I = NaN(3,100);
parfor i=1:100
    outG = sens_timelines_via_refit_cellnum(T,Age,data1,Cell_num_array(:,3),N0,bm,Menarche_Age,coreID, ...
        'DoBaselineGA', true, ...
        'Theta0',theta1,...
        'GuessPerturb',0.2,...
        'Grid',[-1:0.5:1],...
